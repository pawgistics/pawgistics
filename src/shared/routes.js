// @flow

export const HOME_PAGE_ROUTE = '/';
export const LOGIN_PAGE_ROUTE = '/login';
export const USER_MANAGEMENT_PAGE_ROUTE = '/userManagement';
export const FOSTER_MANAGEMENT_PAGE_ROUTE = '/fosterManagement';
export const DOG_DETAIL_PAGE_ROUTE = '/dogDetail/:id';
export const DOGS_PAGE_ROUTE = '/dogs';
export const DOG_MANAGEMENT_PAGE_ROUTE = '/dogManagement';
export const USER_EDIT_PAGE_ROUTE = '/userEdit/:id';
export const RETURN_DOG_PAGE_ROUTE = '/returnDog';
export const USER_DETAIL_PAGE_ROUTE = '/userDetail/:id';
export const ADD_DOG_PAGE_ROUTE = '/addDog';
export const ADD_USER_PAGE_ROUTE = '/addUser';
export const DOG_EDIT_PAGE_ROUTE = '/dogEdit/:id';

export const apiRegisterRoute = '/api/auth/register';
export const apiLoginRoute = '/api/auth/login';
